...
void	init_color_sky(t_data *data)
{
	...
	sorse = mlx_get_data_addr(data->imgs.sky.img, &data->bits_per_pixel, &data->imgs.sky.size_line, &endi);
...
void	init_color_flor(t_data *data)
{
	...
	sorse = mlx_get_data_addr(data->imgs.flor.img, &data->bits_per_pixel, &data->imgs.flor.size_line, &endi);
